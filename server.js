...
function handleDisconnect(ws) {
    // Remove the client from the main room and any breakout room they might be in
    const { roomId, breakoutRoomId } = ws;

    if (roomId) {
        const room = rooms[roomId];
        if (room) {
            // Filter out the disconnected client from the main room
            room.clients = room.clients.filter(client => client.ws !== ws);

            // If the client was in a breakout room, also remove them from there
            if (breakoutRoomId && room.breakoutRooms[breakoutRoomId]) {
                const breakoutRoom = room.breakoutRooms[breakoutRoomId];
                breakoutRoom.clients = breakoutRoom.clients.filter(client => client.ws !== ws);

                // If the breakout room is now empty, optionally delete it
                if (breakoutRoom.clients.length === 0) {
                    delete room.breakoutRooms[breakoutRoomId];
                }
            }
    ...
    console.log(`${ws.name} has disconnected from room ${roomId}`);
}
